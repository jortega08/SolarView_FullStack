...
class InAppBackend(NotificationBackend):
    ...
    def send(self, notificacion) -> None:
        logger.info(
            ...
            notificacion.idnotificacion, notificacion.usuario_id, notificacion.asunto,
        )
...
class EmailBackend(NotificationBackend):
    ...
    def send(self, notificacion) -> None:
        ...
        logger.info('email enviado: id=%s → %s', notificacion.idnotificacion, destinatario)
...
class WebhookBackend(NotificationBackend):
    ...
    def send(self, notificacion) -> None:
        ...
        url = (
            (notificacion.metadata or {}).get('webhook_url')
            or getattr(settings, 'DEFAULT_WEBHOOK_URL', '')
        )
        if not url:
        ...
        logger.info(
            ...
            notificacion.idnotificacion, url, resp.status_code,
        )
