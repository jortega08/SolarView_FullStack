...
def forwards(apps, schema_editor):
    ...
    for dom in Domicilio.objects.select_related('usuario', 'ciudad').all():
        ...
        Consumo.objects.filter(domicilio_id=dom.iddomicilio).update(instalacion_id=inst.idinstalacion)
        Bateria.objects.filter(domicilio_id=dom.iddomicilio).update(instalacion_id=inst.idinstalacion)
        Alerta.objects.filter(domicilio_id=dom.iddomicilio).update(instalacion_id=inst.idinstalacion)
        ConfiguracionUser.objects.filter(domicilio_id=dom.iddomicilio).update(instalacion_id=inst.idinstalacion)
