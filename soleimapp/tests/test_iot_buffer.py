...
class TestRegistrarDatosBuffered:
    ...
    def test_sync_mode_returns_200(self, client, instalacion, settings):
        ...
        with patch('telemetria.tasks.process_battery_alerts') as mock_alerts, \
             patch('telemetria.tasks.notify_realtime_update') as mock_notify:
...
class TestFlushIotBuffer:
    ...
    def test_flush_creates_records(self, instalacion):
        ...
        with patch('django_redis.get_redis_connection', return_value=mock_redis), \
             patch('telemetria.task.process_battery_alerts') as mock_alerts:
            mock_alerts.delay = MagicMock()
            flush_iot_buffer()
