...
class TestComparativaEmpresa:
    def _make_consumo(self, instalacion, fuente, energia, costo):
        ...
        offset = Consumo.objects.filter(instalacion=instalacion).count() + 1
        ...
        consumo.fecha = timezone.now() - timedelta(minutes=offset)
        Consumo.objects.filter(pk=consumo.pk).update(fecha=consumo.fecha)
        return consumo
    ...
    def test_correct_aggregates(self, admin_client, empresa, instalacion):
        ...
        row = next(
            (
                r
                for r in data["data"]
                if r["instalacion_id"] == instalacion.idinstalacion
            ),
            None,
        )
