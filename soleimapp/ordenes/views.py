...
class OrdenTrabajoViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = OrdenTrabajo.objects.select_related(
            'instalacion__empresa', 'asignado_a', 'creado_por', 'alerta', 'mantenimiento',
        )
        if user.rol == 'admin':
            return qs
    ...
    def asignar(self, request, pk=None):
        ...
        if sla:
            try:
                ...
                return Response({'error': 'sla_objetivo_horas debe ser entero.'}, status=400)
        ...
        registrar_evento(
            usuario=request.user, accion='asignar_orden',
            entidad='OrdenTrabajo', entidad_id=orden.idorden,
    ...
    def iniciar(self, request, pk=None):
        ...
        registrar_evento(
            usuario=request.user, accion='iniciar_orden',
            entidad='OrdenTrabajo', entidad_id=orden.idorden,
            detalle={'codigo': orden.codigo}, request=request,
        )
        return Response(self.get_serializer(orden).data)
    ...
    def completar(self, request, pk=None):
        ...
        orden.notas_resolucion = (orden.notas_resolucion + '\n' + notas).strip() if notas else orden.notas_resolucion
        ...
        registrar_evento(
            usuario=request.user, accion='completar_orden',
            entidad='OrdenTrabajo', entidad_id=orden.idorden,
            detalle={'codigo': orden.codigo, 'tiempo_horas': orden.tiempo_resolucion_horas()},
            request=request,
        )
        return Response(self.get_serializer(orden).data)
    ...
    def cerrar(self, request, pk=None):
        ...
        registrar_evento(
            usuario=request.user, accion='cerrar_orden',
            entidad='OrdenTrabajo', entidad_id=orden.idorden,
            detalle={'codigo': orden.codigo}, request=request,
        )
        return Response(self.get_serializer(orden).data)
    ...
    def cancelar(self, request, pk=None):
        ...
        orden.notas_resolucion = (orden.notas_resolucion + f'\nCancelada: {motivo}').strip()
        orden.save()

        registrar_evento(
            usuario=request.user, accion='cancelar_orden',
            entidad='OrdenTrabajo', entidad_id=orden.idorden,
            detalle={'codigo': orden.codigo, 'motivo': motivo}, request=request,
        )
    ...
    def comentarios(self, request, pk=None):
        ...
        com = ComentarioOrden.objects.create(
            orden=orden, usuario=request.user, tipo='comentario', texto=texto,
        )
    ...
    def evidencias(self, request, pk=None):
        ...
        if request.method == 'GET':
            ...
            return Response(EvidenciaOrdenSerializer(qs, many=True, context={'request': request}).data)
        ...
        ev = EvidenciaOrden.objects.create(
            orden=orden, tipo=tipo, archivo=archivo,
            descripcion=descripcion, subido_por=request.user,
        )
        return Response(
...
def mis_ordenes(request):
    ...
    """
    qs = OrdenTrabajo.objects.select_related('instalacion').filter(asignado_a=request.user)
