...
def _empresas_visibles(user, roles=('admin_empresa', 'operador')):
    ...
    return list(
        ...
        .filter(usuario=user, rol__in=roles)
...
class ContratoServicioViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        registrar_evento(
            ...
            detalle={'instalacion_id': contrato.instalacion_id, 'nivel': contrato.nivel},
...
class MantenimientoViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        qs = Mantenimiento.objects.select_related('instalacion', 'plan', 'orden_trabajo')
    ...
    def cancelar(self, request, pk=None):
        ...
        if mant.orden_trabajo and mant.orden_trabajo.estado not in ('completada', 'cerrada', 'cancelada'):
