...
def register(request):
    ...
    logger.info('Nuevo usuario registrado: %s (id=%s)', usuario.email, usuario.idusuario)
...
def login(request):
    ...
    if not usuario.is_active:
        logger.warning('Login denegado — cuenta inactiva: %s (id=%s)', email, usuario.idusuario)
        return Response(
    ...
    if usuario.is_locked():
        ...
        logger.warning(
            ...
            email, usuario.idusuario, minutos_restantes,
        )
        return Response(
            {
                'success': False,
    ...
    if not usuario.check_password(contrasena):
        ...
        intentos_restantes = max(0, usuario._MAX_FAILED_ATTEMPTS - usuario.failed_login_attempts)
        logger.warning(
            'Login fallido — contraseña incorrecta: %s (id=%s), intentos restantes: %d',
            email, usuario.idusuario, intentos_restantes,
        )
        registrar_evento(
            usuario=usuario,
            accion='login_failed',
